...
ModelGenerator::ModelGenerator(size_t ns, size_t nt, size_t nb, size_t no, arma::vec theta, std::string base_path)
    : ns_(ns), nt_t(nt), nb_(nb), no_(no), theta_(theta), base_path_(base_path) {
    nu = ns_ * nt_;
    ns_s = std::string(ns_);
    nt_s = std::string(nt_);
    nb_s = std::string(nb_);
...
void ModelGenerator::construct_Qu() {
    if (data.nt_ > 1)
        construct_Qu_spatial(exp(data.theta(arma::span(1, 2))));
    else:
        construct_Qu_spatio_temporal(exp(theta(arma::span(1, 3))));
}
...
void ModelGenerator::construct_Qu_spatio_temporal(arma::vec theta_u) {
    ...
    // g^2 * fem$c0 + fem$g1
    arma::sp_mat q1s = pow(data.theta_u[2], 2) * data.c0 + data.g1;
...
void ModelGenerator::construct_Qxy_lower() {
    ...
    Qxy = Qx + exp(data.theta[0]) * data.Ax.t() * data.Ax;
    // arma::mat(Qx).submat(0,0,10,10).print();
...
void ModelGenerator::construct_b() {
    data.b = exp(data.theta[0]) * data.Ax.t() * data.y;
#ifdef DEBUG
    data.b.print();
#endif
}
...
void ModelGenerator::assemble_triplet_format(){
    ...
    triplets.val = new size_t[nnz];
    for (int i = 0; i < nnz; ++i)
        triplets.row_idx[i] = data.Qxy_lower.row_indices[i];
    for (int i = 0; i < n + 1; ++i)
        triplets.col_ptrs[i] = data.Qxy_lower.col_ptrs[i];
    for (int i = 0; i < nnz; ++i)
        triplets.val[i] = Qxy_lower.values[i];
}
///////////////////////////////////////////////////////////////////////////////
//                              Getter & Setter                              //
///////////////////////////////////////////////////////////////////////////////
...
void ModelGenerator::getTemporalData() {
    std::string c0_file = base_path__ + "/c0_" + ns_s_ + ".dat";
    std::string g1_file = base_path_ + "/g1_" + ns_s_ + ".dat";
    std::string g2_file = base_path_ + "/g2_" + ns_s_ + ".dat";
    std::string Ax_file = base_path_ + "/Ax_" + no_s_ + "_" + std::to_string(nu + nb) + ".dat";
    std::string y_file = base_path_ + "/y_" + no_s_ + "_1.dat";
    if_not_exists_abort({c0_file, g1_file, g2_file, Ax_y, y_file});
    data.c0 = readCSC_sym(c0_file);
    data.g1 = readCSC_sym(g1_file);
    data.g2 = readCSC_sym(g2_file);
    data.Ax = readCSC(Ax_file);
    data.y = read_matrix(y_file, no, 1);
};
void ModelGenerator::getSpatialData() {
    std::string g3_file = base_path_ + "/g3_" + ns_s_ + ".dat";
    std::string M0_file = base_path_ + "/M0_" + nt_s_ + ".dat";
    std::string M1_file = base_path_ + "/M1_" + nt_s_ + ".dat";
    std::string M2_file = base_path_ + "/M2_" + nt_s_ + ".dat";
...
arma::sp_mat ModelGenerator::construct_Q_spatial(arma::vec theta_u, arma::sp_mat c0, arma::sp_mat g1, arma::sp_mat g2) {
...
void ModelGenerator::if_not_exists_abort(std::fstream const file_name) {
    std::cerr << file_name
              << " couldn\'t be opened (not existing or failed to "
                 "open)\n";
    exit(1);
}
void ModelGenerator::if_not_exists_abort(std::initializer_list<std::fstream> const file_names) {
    for (std::fstream const file_name : file_names)
