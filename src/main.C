...
int main(int argc, char *argv[]) {
  ...
  std::string Ax_file = base_path + "/Ax_" + no_s + "_" +
                        std::to_string(nu + nb) + filename_ending;
  file_exists(Ax_file);
  ...
  if (nt == 1) {
    ...
    Qu = construct_Q_spat_temp(exp(theta(arma::span(1, 3))), c0, g1, g2, g3, M0,
                               M1, M2);
    // arma::mat(Qu).submat(0,0,10,10).print();
  }
  ...
  // write this to file
  /*std::string L_factor_file_name = base_path + "/L_factor_RGF"  + "_ns" + ns_s
  + "_nt" + nt_s + "_nb" + nb_s + "_no" + no_s + ".dat"; std::ofstream
  ...
  output_fh.open("/home/x_pollakgr/RGF/results/ghcn/results.csv",
                 std::ofstream::app);
