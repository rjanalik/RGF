...
arma::sp_mat construct_Q_spatial(arma::vec theta_u, arma::sp_mat c0,
    ...
    Qs = pow(theta_u[0], 2) *
         (pow(theta_u[1], 4) * c0 + 2 * pow(theta_u[1], 2) * g1 + g2);
...
//                         arma::sp_mat* c0, arma::sp_mat* g1, arma::sp_mat*
//                         g2){
arma::sp_mat construct_Q_spat_temp(arma::vec theta_u, arma::sp_mat c0,
                                   arma::sp_mat g1, arma::sp_mat g2,
                                   arma::sp_mat g3, arma::sp_mat M0,
                                   arma::sp_mat M1, arma::sp_mat M2) {
    ...
    arma::sp_mat q2s =
        pow(theta_u[2], 4) * c0 + 2 * pow(theta_u[2], 2) * g1 + g2;

    // g^6 * fem$c0 + 3 * g^4 * fem$g1 + 3 * g^2 * fem$g2 + fem$g3
    arma::sp_mat q3s = pow(theta_u[2], 6) * c0 + 3 * pow(theta_u[2], 4) * g1 +
                       3 * pow(theta_u[2], 2) * g2 + g3;

    // assemble overall precision matrix Q.st
    Qst = theta_u[0] * (kron(M0, q3s) + kron(M1 * 2 * theta_u[1], q2s) +
                        kron(M2 * theta_u[1] * theta_u[1], q1s));
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5 + 1) {
        ...
        std::cerr << "[string:base_path]          path to folder containing "
                     "all files "
                  << std::endl;
        std::cerr << "[integer:ns]                number of spatial "
                     "grid points "
                  << std::endl;
        std::cerr << "[integer:nt]                number of temporal "
                     "grid points "
                  << std::endl;
    ...
    // Ax = cbind(As, B)
    std::string Ax_file =
        base_path + "/Ax_" + no_s + "_" + std::to_string(nu + nb) + ".dat";
    file_exists(Ax_file);
    ...
    if (nt == 1) {
        ...
        Qu = construct_Q_spat_temp(exp(theta(arma::span(1, 3))), c0, g1, g2, g3,
                                   M0, M1, M2);
        // arma::mat(Qu).submat(0,0,10,10).print();
    }
    ...
    + ns_s + "_nt" + nt_s + "_nb" + nb_s + "_no" + no_s + ".dat";
    std::ofstream L_factor_file(L_factor_file_name,    std::ios::out |
    std::ios::trunc);
    ...
    std::string log_file_name = base_path + "/log_RGF_ns" + ns_s + "_nt" +
                                nt_s + "_nb" + nb_s + "_no" + no_s + ".dat";
