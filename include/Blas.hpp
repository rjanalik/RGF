/**
 * @file      Blas.hpp
 * @brief     Wrapper file that wraps Fortran Blas function calls into extern C
 * function calls.
...
inline void c_dgemv(char transa, int m, int n, double alpha, double *a, int lda,
  ...
  fortran_name(dgemv, DGEMV)(&transa, &m, &n, &alpha, a, &lda, x, &incx, &beta,
                             y, &incy);
}
...
inline void c_zgemv(char transa, int m, int n, CPX alpha, CPX *a, int lda,
  ...
  fortran_name(zgemv, ZGEMV)(&transa, &m, &n, &alpha, a, &lda, x, &incx, &beta,
                             y, &incy);
}
...
inline void tgemm_dev(char transa, char transb, int m, int n, int k,
  ...
  magma_dgemm(magma_transa, magma_transb, m, n, k, alpha, a, lda, b, ldb, beta,
              c, ldc, queue);
}
  ...
  if (type) {
    ...
    /*
      magma_zgesv_nopiv_gpu(n,nrhs,(magmaDoubleComplex_ptr)a,lda,(magmaDoubleComplex_ptr)b,\
                            ldb,info);
    */
    magma_zhesv_nopiv_gpu(MagmaLower, n, nrhs, (magmaDoubleComplex_ptr)a, lda,
                          (magmaDoubleComplex_ptr)b, ldb, info);
  }
}
...
inline void c_dgeev(char jobvl, char jobvr, int n, double *a, int lda,
  ...
  fortran_name(dgeev, DGEEV)(&jobvl, &jobvr, &n, a, &lda, wr, wi, vl, &ldvl, vr,
...
inline void c_zggev(char jobvl, char jobvr, int n, CPX *a, int lda, CPX *b,
  ...
  fortran_name(zggev, ZGGEV)(&jobvl, &jobvr, &n, a, &lda, b, &ldb, alpha, beta,
...
inline void c_zheev(char jobvl, char uplo, int n, CPX *a, int lda, double *w,
  ...
  fortran_name(zheev, ZHEEV)(&jobvl, &uplo, &n, a, &lda, w, work, &lwork, rwork,
                             info);
}
...
inline void c_dstebz(char *range, char *order, int *iter, double *vl,
                     ...
                     int *info) {
  fortran_name(dstebz, DSTEBZ)(range, order, iter, vl, vu, il, iu, abstol, diag,
                               offd, neval, nsplit, eval, iblock, isplit, work,
                               iwork, info);
}
...
inline void extract_not_diag_on_dev(double *D, int *edge_i, int *index_j,
                                    ...
                                    cudaStream_t stream) {
  d_extract_not_diag_on_dev(D, edge_i, index_j, nnz, NR, imin, imax, jmin, side,
                            shift, findx, stream);
}
...
inline void extract_not_diag_on_dev(CPX *D, int *edge_i, int *index_j, CPX *nnz,
                                    ...
                                    int side, int shift, int findx,
                                    cudaStream_t stream) {
  z_extract_not_diag_on_dev(D, edge_i, index_j, nnz, NR, imin, imax, jmin, side,
                            shift, findx, stream);
}
...
inline void init_supernode_dev(T *M, size_t *ia, size_t *ja, T *a,
                               ...
                               size_t nd, cudaStream_t stream = NULL);
...
inline void init_supernode_dev(double *M, size_t *ia, size_t *ja, double *a,
  ...
  d_init_supernode_on_dev(M, ia, ja, a, supernode, supernode_nnz,
                          supernode_offset, ns, nt, nd, stream);
}
